...
export const GET: APIRoute = async () => {
  ...
  const urls = [...new Set(['/', ...Object.keys(pages), ...journalPages, ...services.map(([, href]) => href), ...nonPostRecords.map((record) => record.path), ...posts.map((post) => `/${post.slug}/`)])].filter((path) => !excludedPaths.has(path));

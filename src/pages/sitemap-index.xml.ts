...
export const GET: APIRoute = async () => {
  ...
  const excludedPaths = new Set(['/home/', '/test/', '/thank-you/', '/collections/']);
  const urls = [...new Set(['/', ...Object.keys(pages), ...journalPages, ...services.map(([, href]) => href), ...nonPostRecords.map((record) => record.path), ...posts.filter((post) => !post.seo?.noIndex).map((post) => `/${post.slug}/`)])].filter((path) => !excludedPaths.has(path));
